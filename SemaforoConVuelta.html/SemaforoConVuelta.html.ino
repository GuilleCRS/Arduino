...
int tiempoVerde=5000;//Tiempo que dura el led verde previo al cambio de luz y posterior a la finalizacion de la vuelta 
...
void loop() {
  ...
  manejoLuces(led[0],led[1],led[2],led[3]);
  
    //Se mandan los leds del semaforo 2 para que se haga el manejo correspondiente
  manejoLuces(led[4],led[5],led[6],led[7]);
}
/* El siguiente metodo hace parpadear un led 4 veces como indicacion de cambio por un tiempo definido en
 * la variable tiempoCambio donde la espera entre apagado seria de la mitad de dicha variable */
void cambio(int led){
  for(int i=0;i<3;i++){
...
/* El siguiente metodo se encarga de recibir las luces de un semaforo para asi
 * realizar el control a como es debido dependiendo las variables declaradas */
void manejoLuces(int verde,int vuelta,int amarillo,int rojo){
  //Enciende el led verde del semaforo en cuestion
  digitalWrite(verde,HIGH);
  
  //Si el rojo actual es igual a 11 estariamos tratando con el segundo semaforo entonces se adapta para que el rojo sea del contrario
  if(rojo==11){
    rojo-=4;
  }else{
    rojo+=4;
  ...
  //Enciende la vuelta verde del semaforo en cuestion, hace una espera del tiempo que se desea que dure la vuelta y hace un cambio de luz antes de apagarse
  digitalWrite(vuelta,HIGH);
  delay(tiempoVuelta);
  cambio(vuelta);
  
  //Es el tiempo que va a durar el verde despues de que se finaliza la vuelta para posteriormente hacer la señal de cambio
  delay(tiempoVerde);
  cambio(verde);
  
  //Dado que en el cambio el led es apagado se inicia el amarillo y se hace una espera del tiempo predefinido en la variable tiempoAmarillo 
